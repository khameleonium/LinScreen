...
Модуль проверяет тип сессии (поддерживается только X11), переменные
окружения и наличие необходимых утилит в системе.
...
class DesktopSession:
    ...
    # Название окружения рабочего стола в нижнем регистре: x-cinnamon, xfce, mate.
    desktop: str
    ...
    def supports_native_hotkeys(self) -> bool:
        """Признак возможности прямого перехвата клавиш в X11."""
        return self.is_x11

    @property
    def screenshot_backend(self) -> str:
        """Способ получения снимка экрана."""
        return "qt"

    @property
    def region_backend(self) -> str:
        """Способ выделения области экрана."""
        return "overlay"

    @property
    def video_backend(self) -> str:
        """Способ захвата видеопотока."""
        return "x11grab"
