...
Приложение не изобретает кодеки и не тянет лишних зависимостей: захват и
кодирование выполняет FFmpeg (x11grab), звук берётся у PulseAudio или
PipeWire-Pulse, а снимки экрана и границы окон считываются нативно через X11.
Всё это упаковано в один запускаемый файл, которому не нужны ни Python, ни Qt,
ни установленный в системе ffmpeg.
...
* выделенная область, весь экран, отдельный монитор;
* автоматическое выделение окна или элемента под курсором;
...
размытие, текст с подложкой, круговой нумератор шагов и кадрирование (обрезка).
Кнопки панели снабжены векторными пиктограммами и подсказками с клавишами
быстрого выбора. Готовые аннотации перемещаются мышью, действия отменяются
и возвращаются. Размытие необратимо уничтожает содержимое области, а не
закрывает его сверху, — годится для скрытия паролей и личных данных.
...
* рамка вокруг записываемой области, не попадающая в запись;
...
| Весь экран, отдельный монитор | пункты меню |
| Объект под курсором | пункт меню либо наведение при выделении области |
| Отмена выделения | `Esc`, правая кнопка мыши |
| Выбрать весь экран во время выделения | `Enter` |

**Объект под курсором.** При наведении указателя программа сама находит
окно или элемент под ним и обводит его зелёным пунктиром, показывая
размер. Щелчок снимает подсвеченное. Если вместо щелчка протянуть мышь,
автоматика уступает: снимается собственноручно выделенная область.

Свёрнутые и перекрытые окна не учитываются: подсвечивается то, что действительно
видно на экране под указателем, с соблюдением реального Z-порядка окон. Внутренние
элементы определяются не во всех приложениях — современные наборы виджетов рисуют
содержимое окна целиком и отдельных окон для кнопок и панелей не создают.
...
| Действие | Клавиши |
|---|---|
| Выбор инструмента | `1`…`8` в порядке кнопок на панели |
| Применить обрезку (кадрирование) | `Enter` или двойной щелчок |
| Отменить текущую обрезку | `Esc` |
...
Границы записываемой области обведены рамкой: зелёной при выделении,
красной во время записи и жёлтой на паузе. Рамка рисуется снаружи
области и в запись не попадает.
...
Модификаторы учитываются точно: сочетание `Ctrl+Print` не вызовет
действие, назначенное на одиночный `Print`.

Изменяются в «Настройки → Клавиши», поле заполняется самим нажатием.
На время набора перехват приостанавливается, поэтому нажатие в поле не
...
Подробное описание для тех, кто будет дорабатывать программу, — в
[FOR_DEVs.md](FOR_DEVs.md): карта модулей, действующие соглашения,
архитектурные решения X11-контура и ориентиры для разработки.

```
core/     настройки, определение сессии, хоткеи, автозапуск, фоновые вызовы
capture/  захват экрана X11, дерево окон (python-xlib), звуковые устройства
encoder/  профили FFmpeg, контроллер процессов, сохранение изображений
editor/   графический холст аннотаций
ui/       трей, оверлей выделения, рамка записи, панель, настройки, журнал
...
## Архитектурная концепция (X11 Edition)

LinScreen ориентирован исключительно на графическую подсистему **X11**.

Отказ от попыток универсальной кроссплатформенности — осознанное решение:
* В X11 приложение имеет полный, прямой и быстрый доступ к экрану: оверлей
  размещается поверх всех окон без согласований с оконным менеджером,
  дерево окон (`python-xlib`) мгновенно определяет границы активных элементов
  под курсором, а FFmpeg ведёт запись через нативный демультиплексор `x11grab`.
* Сессии Wayland принципиально изолируют приложения друг от друга, запрещают
  прямой доступ к глобальным координатам мыши и требуют внешних порталов
  (ScreenCast, PipeWire, GlobalShortcuts), чья реализация фрагментирована и
  нестабильна в разных окружениях (Cinnamon, GNOME, KDE).

Вместо компромиссного «комбайна» с костылями и взаимными ограничениями
кодовая база LinScreen остаётся чистым, эталонным решением для X11.
Под Wayland и другие графические оболочки в дальнейшем будут создаваться
отдельные, независимые приложения с нативной архитектурой.
